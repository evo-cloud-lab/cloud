...
function serviceExec(opts) {
    ...
    opts.config && opts.config.forEach(function (cfgfile) { args.push('-c'); args.push(cfgfile); });
    opts.define && opts.define.forEach(function (def) { args.push('-D'); args.push(def); });

    cli.debugging && cli.logOut('Arguments: %j', args);
    Config.conf(args, { reloadSignal: true });
    cli.debugging && cli.logOut('Configurations: %j', Config.conf().opts);
...
var STATUS_STYLUS = {
    subject: function (name) { return cli.pad(name, 16); },
    state: function (state) {
        return { RUNNING: cli.ok, STOPPED: cli.err, INVALID: cli.lo }[state].call(cli, state);
    }
};
...
function serviceStatus(opts) {
    ...
    names.forEach(function (reqName) {
        ...
        if (service) {
            ...
            cli.logAction(null, name, pid ? 'RUNNING' : 'STOPPED', STATUS_STYLUS);
        } else {
            cli.logAction(null, name, 'INVALID', STATUS_STYLUS);
        }
    });
}

function serviceList(opts) {
    cli.logList(Object.keys(cli.services));
}
